...
-- Shared targeting includes humans, combat use and all three lotus tiers.
local function ConsiderHealingLotus(item)
    local target = LotusUsage.Target(bot, item)
    if target then return BOT_ACTION_DESIRE_HIGH, target, 'unit', 'Lotus restoration' end
    return BOT_ACTION_DESIRE_NONE
end
X.ConsiderItemDesire['item_famango'] = ConsiderHealingLotus
X.ConsiderItemDesire['item_great_famango'] = ConsiderHealingLotus
X.ConsiderItemDesire['item_greater_famango'] = ConsiderHealingLotus
...
X.ConsiderItemDesire["item_tpscroll"] = function( hItem )
    ...
    if nMode == BOT_MODE_RUNE or (J.IsDoingRoshan(bot)
        and GetUnitToLocationDistance(bot, J.GetCurrentRoshanLocation()) <= 2800) then
        return BOT_ACTION_DESIRE_NONE
    end
